Settings Property   
  
[DriveWorks.SolidWorks Assembly](topic13342.md) > [DriveWorks.SolidWorks Namespace](topic13345.md) > [FileFormatGenerator Class](topic13579.md) : Settings Property  
---  
  
Visual Basic (Declaration)    
Visual Basic (Usage)    
C# 

Glossary Item Box

Gets/sets the generation settings that should be used. 

# Syntax

Visual Basic (Declaration)|   
---|---  
      
    
    Protected Property Settings As [GenerationSettings](topic15238.md)  
  
Visual Basic (Usage)| Copy Code  
---|---  
      
    
    Dim instance As [FileFormatGenerator](topic13579.md)
    Dim value As [GenerationSettings](topic15238.md)
     
    instance.Settings = value
     
    value = instance.Settings  
  
C#|   
---|---  
      
    
    protected [GenerationSettings](topic15238.md) Settings {get; set;}  
  
# Requirements

**Target Platforms:** Please see DriveWorks software prerequisites.

# See Also

#### Reference

[FileFormatGenerator Class](topic13579.md)   
[FileFormatGenerator Members](topic13580.md)


